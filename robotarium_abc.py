...
class RobotariumABC(ABC):
    ...
    def _validate(self, errors = {}):
        ...
        for i in range(N):
            ...
            if(x < b[0] or x > (b[0] + b[2]) or y < b[1] or y > (b[1] + b[3])):
                if "boundary" in errors:
                    errors["boundary"] += 1
                    errors["boundary_positions"].append([x, y]) # store the positions that are outside of boundary
                else:
                    errors["boundary"] = 1
                    errors["boundary_string"] = "iteration(s) where robots were outside the boundaries."
                    errors["boundary_positions"] = []
            ...
            for k in range(j+1,N):
                if(np.linalg.norm(p[:2,j]-p[:2,k]) <= self.robot_diameter):
                    if "collision" in errors:
                        errors["collision"] += 1
                        errors["collision_positions"].append([p[:2,j], p[:2,k]]) # store the positions that have collision
                    else:
                        errors["collision"] = 1
                        errors["collision_string"] = "iteration(s) where robots collided. The distance between robots is less than 0.11 meter."
                        errors["collision_positions"] = []
        ...
        #if np.any(exceeding):
        #    print("DXDD:", dxdd)
        #    print("Absoluate dxdd:", np.absolute(dxdd))
        #    print("Maximum Velocity is:", self.max_wheel_velocity)
